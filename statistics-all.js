//COPY PARTY, JOIN ATTENDANCE HERE//
//----GLOBAL VARIABLES----//
...
function init() {
    ...
    //---ARRAY ORDERED BY VOTES_PARTY_PCT---//
    ...
    //--FUNCTION COMPARE LAST 11% (ARRAY) WITH all ARRAYMEMBERS--//
    function compare() {
        for (var i = arrayMemberT.length; i < arrayMember.length; i++) {
            if (arrayMemberT[arrayMemberT.length - 1].votes_with_party_pct == arrayMember[i].votes_with_party_pct) {
    ...
    //---INVERSE ORDER BY VOTES_PARTY_PCT---//
    arrayMember.sort(function (a, b) {
        if (a.votes_with_party_pct > b.votes_with_party_pct) {
            return 1;
        }
    ...
    function compare() {
        for (var i = arrayMemberB; i < arrayMember.length; i++) {
            if (arrayMemberB[arrayMemberB.length - 1].votes_with_party_pct == arrayMember[i].missed_votes_pct) {
...
function newTable(pArray) {
    ...
    for (var i = 0; i < pArray.length; i++) {
        ...
        td1.innerHTML = pArray[i].total_votes;
        tr.appendChild(td1);
